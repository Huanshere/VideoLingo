...
- LLM：推薦 [OpenLux](https://www.openlux.ai/register?aff=wKYu)（`https://api.openlux.ai/v1`）。優先 `gpt-6-luna`（較便宜），品質更好用 `gpt-6-sol`，品質最好用 `claude-opus-5-5`。其他能回傳流程所需結構化 JSON 的 OpenAI 相容服務也可使用。在側欄設定 API 網址、金鑰和模型。
