...
- LLM：推荐 [OpenLux](https://www.openlux.ai/register?aff=wKYu)（`https://api.openlux.ai/v1`）。优先 `gpt-6-luna`（更便宜），质量更好用 `gpt-6-sol`，质量最好用 `claude-opus-5-5`。其他能返回流程所需结构化 JSON 的 OpenAI 兼容服务也可使用。在侧栏配置 API 地址、密钥和模型。
