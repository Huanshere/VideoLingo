...
| BUILDER-305 (unreleased) | Default local ASR becomes Qwen3-ASR + Qwen3-ForcedAligner. Removes whisperx, pyannote-audio, ctranslate2 and torchcodec from `requirements.txt` and the installer's WhisperX stage (installer now has 6 stages). Adds marker-split engines: `qwen-asr==0.0.6` with Transformers 4.57/Hub <1 outside Apple Silicon; `mlx-audio>=0.5.5,<0.6` with Transformers 5/Hub 1 plus nagisa/soynlp on Apple Silicon (macOS ≥14). Torch 2.8 family unchanged. WhisperX is an optional manual install (`docs/pages/docs/whisperx-optional.*.md`); on Apple Silicon the installer removes whisperx/torchcodec (hub <1 conflict). The installer stops early on Intel Macs and on Apple Silicon below macOS 14. |
