...
*   `core/asr_backend/whisperX_local.py`: Manual backend (`whisper.backend: whisperx`). The installer does not install it; see [WhisperX (manual install)](whisperx-manual.en-US.md). Implements local audio transcription using the WhisperX library. Optimizes performance based on available hardware (GPU/CPU), handles model downloads (with mirror checking), performs transcription and alignment, adjusts timestamps, and manages GPU memory.
...
*   `core/_2_asr.py`: Orchestrates audio preparation, optional vocal separation, recognition with local Qwen3-ASR (default), a manually installed WhisperX backend, or ElevenLabs, content-addressed result caching, and result export.
